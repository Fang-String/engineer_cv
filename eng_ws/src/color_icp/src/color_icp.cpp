...
class ColorICP {
  public:
    ...
    ColorICP (const Config &conf, std::shared_ptr<rclcpp::Node> nh_);
    ~ColorICP () {}
...
ColorICP::ColorICP (const Config &conf, std::shared_ptr<rclcpp::Node> nh_)
  ...
  // load point clouds
  if(!config.precloud){
  std::string src_filename = config.src_cloud;
  std::string tar_filename = config.tar_cloud;
  ...
  // registration
  int icp_method = config.icp_method;
  if (icp_method == 0)
...
void ColorICP::downSampleVoxelGrids (const PointCloudPtr& cloud) {
  double resolution = config.voxel_resolution;
...
void ColorICP::estimateNormals (const PointCloudPtr& cloud) {
  ...
  ne.setRadiusSearch (config.normal_est_radius);
...
//   if (!source_transformed || !target) {
//     std::cerr << "Error: Point clouds are not initialized!" << std::endl;
//     return;
// }
// while (!visualizer.wasStopped()) {
//     visualizer.spinOnce();
...
//   std::cout<<"\n!!!!\n";
 if (!source_transformed || !target) {
        std::cerr << "Error: Input point clouds are invalid." << std::endl;
        return;
    }
    ...
    // 添加点云到可视化器
    if (visualizer.addPointCloud<PointT>(source_transformed, "source cloud transformed")) {
        std::cout << "Source cloud added successfully." << std::endl;
    } else {
        std::cerr << "Failed to add source cloud." << std::endl;
    }

    if (visualizer.addPointCloud<PointT>(target, "target cloud")) {
        std::cout << "Target cloud added successfully." << std::endl;
    } else {
        std::cerr << "Failed to add target cloud." << std::endl;
    }

    // 主循环
    while (!visualizer.wasStopped()) {
        visualizer.spinOnce();
        pcl_sleep(10);  // 确保这个函数实现正确
    }
...
Eigen::Matrix4d ColorICP::PCLICP (const PointCloudPtr& source, const PointCloudPtr& target) {
  ...
  icp.setMaxCorrespondenceDistance (config.icp_max_corres_dist);
...
Eigen::Matrix4d ColorICP::ClassicICPRegistration (const PointCloudPtr& source, const PointCloudPtr& target) {
  // initialization
  int iteration = config.icp_max_iterations;
  double translation_epsilon = config.icp_max_iterations;
  double rotation_epsilon = 1.0 - config.icp_transformation_epsilon;
  double distance_threshold = config.icp_max_corres_dist; // icp.setMaxCorrespondenceDistance
...
Eigen::Matrix4d ColorICP::ColorICPRegistration (const PointCloudPtr& source, const PointCloudPtr& target) {
  // initialization
  int iteration = config.icp_max_iterations;
  double translation_epsilon =  config.icp_transformation_epsilon;
  double rotation_epsilon = 1.0 - config.icp_transformation_epsilon;
  double distance_threshold = config.icp_max_corres_dist; // icp.setMaxCorrespondenceDistance
  int cloud_size = static_cast<int> (source->size());
  Eigen::Matrix4d final_transformation = Eigen::Matrix4d::Identity();
...
void ColorICP::prepareColorGradient (const PointCloudPtr& target) {
  ...
  double search_radius = config.search_radius;
...
Eigen::Matrix4d ColorICP::GaussNewtonWithColor (const PointCloudPtr& source, const PointCloudPtr& target, 
  ...
  double lambda = config.color_icp_lambda; // TODO: test varying param values
...
int main(int argc, char** argv) {
    ...
    auto node = std::make_shared<rclcpp::Node>("color_icp", rclcpp::NodeOptions());

    ColorICP global_planner(Config(node), node);
